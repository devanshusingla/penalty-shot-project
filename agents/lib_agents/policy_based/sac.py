...
class SAC:
    ...
    def __call__(self, actor_lr = 0.001, critic_lr = 0.001, tau=0.005, gamma=1.0, n_step=4, **kwargs):
        ...
        actor = ActorProb(actor_net, self.action_shape, self.actor_hidden_shape, device=self.device).to(self.device)
        actor_opt = torch.optim.Adam(actor.parameters(), lr=actor_lr)

        critic_net1 = Net(self.state_shape, self.action_shape, hidden_sizes=self.critic_hidden_shape, concat=True, device=self.device)
        critic1 = Critic(critic_net1, device=self.device).to(self.device)
        critic_opt1 = torch.optim.Adam(critic1.parameters(), lr=critic_lr)

        critic_net2 = Net(self.state_shape, self.action_shape, hidden_sizes=self.critic_hidden_shape, concat=True, device=self.device)
