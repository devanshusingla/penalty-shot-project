...
def make_discrete_env(k:int = 7):
    return DiscreteActionWrapper(gym.wrappers.FlattenObservation(gym.make('gym_env:penalty-shot-v0')), k=k)

def make_render_discrete_env(k:int = 7):
    return DiscreteActionWrapper(gym.wrappers.FlattenObservation(RenderEnvWrapper(gym.make('gym_env:penalty-shot-v0'))), k=k)
